...
class StarPanelState extends State<StarPanel> {
  ...
  Future setNewRate(String request, int rate) async {
    if (request == "create") await RateDbManager().addNewMark(recipeId: widget.id, userUid: UserDB.uid, mark: rate);
    if (request == "update") await RateDbManager().updateMark(id: 0, userUid: UserDB.uid ?? "", recipeId:  widget.id, mark: rate.toDouble());
    if (request == "delete") await RateDbManager().deleteMark(widget.id);
    print("((((((((((((((((((new_rate))))))))))))))))))");
    double new_rate = await RateDbManager().getMark(widget.recipe.id);
      ...
      child: Row(
        ...
        children: [RatingBar.builder(
          ...
          onRatingUpdate: (rating) {
            if (_initialRating == 0) {
              setNewRate("create", rating.toInt());
              if (!ServiceIO.loggedIn) {
                ServiceIO.showMarkInviteDialog(context);
              }
            } else {
            setNewRate("update", rating.toInt());
            if (!ServiceIO.loggedIn) {
              ServiceIO.showMarkInviteDialog(context);
            }
          }
